          ...
          return html`
            <c-div p-4 border-box flex flex-col row-gap-4>
              <c-div
                ...
                round-6
                h-44
                style="background: var(--${this.setColorRequestType(dayPart as DayPart)})">
                <c-div>
                  <c-div
                    class="icon-daypart-sr"
                    .$icon="${dayPortValue[dayPart as DayPart].src}: tx-${dayPortValue[
                      dayPart as DayPart
                    ].size} tx-${dayPortValue[dayPart as DayPart].iconColor}"
                    flex
                    flex-col>
              ...
              <div
                ...
                <c-div>
                  <c-div
                    ...
                    <c-div
                      >${plansEntries.map(
                        ([plan]) => html`<c-div inline tx-12>${plan}</c-div> `
                      )}</c-div
                    >
                  </c-div>
                </c-div>
              ...
              <cx-button
                ...
                @click="${() => {
                  ...
                  if (cellId) {
                    ...
                    const targetElement = event?.target as HTMLElement;
                    const shiftType = targetElement.closest('c-div[shift-type]') as HTMLElement;
                    const requestHostType = shiftType.getAttribute('shift-type');
